...
public class RemoteQueueInteractor {
	...
	public RemoteQueueInteractor() throws NamingException, JMSException {
		...
		/*
		 * The URL below should point to the your instance of Server 1, if
		 * no port offset is used for Server 1 the port can remain at 4447
		 */
		props.put(Context.PROVIDER_URL, "remote://192.168.1.20:4447"); 
		
		/*
		 * Please note that the credentials passed in here have no 
		 * effect on the messaging system as we have disabled the 
		 * security on the HornetQ messaging subsystem
		 */
		...
		/* The following two lookups are based on how you configured the RemoteConnectionFactory 
		 * and the local queue on Server 1. If you have followed the installation that was 
		 * provided as-is then you can go with the below code
		 */
